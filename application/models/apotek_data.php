...
class Apotek_data extends CI_Model
{
    ...
    {
        $data = array();
        $query = $this->db->get('table_pem')->result_array();
    ...
    function getmedbysupplier($nama_pemasok){
     ...
     function getmedbypembeli($nama){
        $hasil=$this->db->query("SELECT * FROM table_pem WHERE nama='$nama'");
        return $hasil->result();
    }
...
function count_pem(){       
      $cp =  $this->db->query('SELECT * FROM table_pem'); 
        $sup = $cp->num_rows();
        return $sup;    
    }
