...
class Example extends CI_Controller
{
	...
	function index() {
		...
		$this->template->write('header', 'Sistem Informasi Djogja Karya MAndiri');
	...
	function table_exp() {
		...
		$this->template->write('title', 'barang kedaluwarsa', TRUE);
		$this->template->write('header', 'Sistem Informasi Djogja Karya MAndiri');
	...
	function table_stock() {
		...
		$this->template->write('title', 'barang Habis', TRUE);
		$this->template->write('header', 'Sistem Informasi Djogja Karya MAndiri');
...
function table_pem() {
		$data['table_pem'] = $this->apotek_data->pembeli()->result();
		
		$this->template->write('title', 'Lihat pembeli', TRUE);
		$this->template->write('header', 'Sistem Informasi Djogja Karya MAndiri');
		$this->template->write_view('content', 'tes/table_pem', $data, true);
	...
	function form_pem() {
		$this->template->write('title', 'tambah pembeli', TRUE);
		$this->template->write('header', 'Sistem Informasi Djogja Karya MAndiri');
		$this->template->write_view('content', 'tes/form_pem', '', true);
	...
	function form_invoice() {
		$data['table_med'] = $this->apotek_data->medicine()->result();
		$data['get_pem'] = $this->apotek_data->get_pembeli();
$data['get_alamat'] = $this->apotek_data->get_alamat();
		$data['get_cat'] = $this->apotek_data->get_category();
	...
	function getmedbypembeli(){
        $nama=$this->input->post('nama');
        $data=$this->apotek_data->getmedbypembeli($nama);
        echo json_encode($data);
    }
	...
	function add_pembeli(){
		...
		$this->apotek_data->insert_data($data,'table_pem');

		$this->session->set_flashdata('pem_added', 'Pembeli berhasil ditambahkan');
		redirect('example/table_pem');
	}
	...
	function edit_form_pem($id_pem) {
		$where = array('id_pem' => $id_pem);
		$data['table_pem'] = $this->apotek_data->edit_data($where,'table_pem')->result();
		$this->template->write('title', 'Ubah pembeli', TRUE);
		$this->template->write('header', 'Sistem Informasi Djogja Karya MAndiri');
		$this->template->write_view('content', 'tes/edit_form_pem', $data, true);
	...
	function update_pembeli(){
		...
		$this->apotek_data->update_data($where,$data,'table_pem');

		$this->session->set_flashdata('sup_pem', 'Data pembeli berhasil diperbarui');
		redirect('example/table_pem');
	}
	...
	function remove_pem($id_pem){
		$where = array('id_pem' => $id_pem);
		$this->apotek_data->delete_data($where,'table_pem');
		redirect('example/table_pem');
	}
